...
**Rotação segura de refresh token**: `refresh-token.service` consome o token via `deleteByUserIdAndRefreshToken` (deleteMany atômico cuja contagem funciona como trava otimista) antes de emitir o novo par — impede double-spend (duas sessões a partir de um único refresh token) em requisições concorrentes.
