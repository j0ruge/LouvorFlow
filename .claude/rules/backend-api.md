...
**Erros do Prisma nunca devem escapar crus.** O handler genérico de `app.ts` ecoa `err.message` fora de produção, então um `P2002`/`P2003`/`P2025` não tratado vira um 500 que vaza detalhe interno. Toda escrita sujeita a corrida deve traduzir os códigos conhecidos em `AppError`. Para o caso "linha sumiu durante a operação", prefira `updateMany` + checagem de `count === 0` lançando um sentinela, em vez de `update` (que lança `P2025` cru) — ver `eventos.repository.setMusicaVersaoAtomic` e seu irmão `setMusicaTonalidadeAtomic` (tom por música na escala, F10 — mesmo padrão).

A tradução em si mora num lugar só: **`traduzirErroDeEscrita(error, mapa)`** (`eventos.service.ts`), que recebe as `sentinelas` daquele caminho (mensagem do `throw new Error('X')` do repositório → `{ mensagem, status }`) e as `fks` reconhecidas (substring do nome da constraint → mensagem, avaliadas na ordem declarada). `P2002` vira sempre 409 "Registro duplicado"; `P2003` com constraint não mapeada cai no 404 genérico "Recurso referenciado não encontrado", para não vazar detalhe interno; o que não for reconhecido é relançado. Os três handlers de eventos (`handleDuplicarSentinel`, `handleVersaoSentinel`, `handleIntegranteSentinel`) hoje só declaram a própria tabela e delegam — antes eram três cópias da mesma estrutura.

Para os **6 domínios de suporte** (artistas, categorias, funções, grupos de funções, tipos de evento e tonalidades — todos com `@@unique([tenant_id, nome|tom])`) o equivalente é **`comBarreiraDeDuplicidade(mensagem, escrita)`** (`src/utils/duplicidade.ts`), que envolve o `create`/`update` e converte `P2002` em 409 com a mensagem da entidade. A checagem prévia `findByNome`/`findByTom` continua existindo (ela dá o erro amigável no caso comum), mas não é trava: duas requisições concorrentes passam juntas por ela e a perdedora bate no índice único — antes desta barreira, esse `P2002` subia cru.

### FK de catálogo exige checagem de tenant explícita
...
Dentro de uma transação interativa, **não** troque um laço sequencial por `Promise.all`: todas as queries da transação correm na mesma conexão, então não há paralelismo real a ganhar — só se troca a falha imediata do item problemático por uma rejeição em lote, com risco de `P2028 Transaction already closed` sob carga. Ver o laço de integrantes em `duplicarEvento`.

A **origem é revalidada dentro da transação** (`ORIGEM_NOT_FOUND`, traduzido em 404). A checagem do service acontece antes de a transação abrir: se o evento for excluído nesse intervalo, a cascata leva junto `Eventos_Musicas`/`Eventos_Users`, as duas leituras voltam vazias e — como nada na cópia aponta para a origem — o Postgres não recusaria nada. Sem a revalidação, a corrida virava um 201 com escala vazia em vez de 404.
