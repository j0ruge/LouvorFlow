...
- [X] T026 [US3] Create `src/backend/src/repositories/integrantes.repository.ts` — extract Prisma queries: `findAll()` with funcoes include, `findById(id)` with funcoes include, `findByDocId(doc_id)`, `findByDocIdExcludingId(doc_id, excludeId)`, `create(data)` with INTEGRANTE_PUBLIC_SELECT, `update(id, data)` with INTEGRANTE_PUBLIC_SELECT, `delete(id)`, `findFuncoesByIntegranteId(integranteId)`, `findIntegranteFuncao(fk_integrante_id, funcao_id)`, `createIntegranteFuncao(fk_integrante_id, funcao_id)`, `deleteIntegranteFuncao(id)`. Read existing `integrantesController.ts` for exact select/include structures.
